...
description: "System that translates domain names into IP addresses to establish a connection between a client and a server."
date: "2025-02-13"
---
...
Root servers (.) → Directs the query to the .com server.
TLD servers (.com) → Directs the query to google.com's name server.
Authoritative server for google.com → Returns the final IP address.
