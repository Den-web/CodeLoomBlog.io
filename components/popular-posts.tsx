...
export function PopularPosts({ posts, title = 'Popular on LinkedIn' }: PopularPostsProps) {
  ...
  return (
    <section className="mb-8 pb-6 border-b border-zinc-200 dark:border-zinc-800">
        ...
        {popular.map(post => {
          ...
          return (
            <li key={post._id}>
              ...
              {stats && <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">{stats}</p>}
              {post.linkedinUrl && (
                <a
